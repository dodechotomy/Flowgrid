...
class Palette {
  ...
  static randomGradient() {
    ...
    //normalize the spans so that there is a good amount of variety
  ...
  split(){
    let secondPalette = new Palette();
    if(this.colors.length < 4){
      secondPalette.colors = [...this.colors];
    }else{
      let halfWayIndex = this.colors.length / 2;
      secondPalette.colors = this.colors.splice(halfWayIndex);
    }
...
Palette.bwPlusAccent = ()=>{
  colorMode(HSB, 1);
  let h = Math.random();
  return new Palette([color(0), color(1), color(h,1,1)]);
}
