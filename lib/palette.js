...
class Palette {
  ...
  static createPalette() {
    let paletteTypes = [{
      ...
      value: Palette.sevenColorPalette,
      weight: 2
    }, {
      value: Palette.inkyColorPalette,
      weight: 2
    },
  ...
  static contrast(foreground,background){

    let rgb1 = [red(foreground), green(foreground), blue(foreground)];
    let rgb2 = [red(background), green(background), blue(background)];
    ...
    const luminanceFore = luminanace(rgb1[0], rgb1[1], rgb1[2]);
    const luminanceBack  = luminanace(rgb2[0], rgb2[1], rgb2[2]);
    return luminanceBack < luminanceFore 
      ? ((luminanceFore + 0.05) / (luminanceBack + 0.05))
      : ((luminanceBack + 0.05) / (luminanceFore + 0.05));
  }


  static findMaximumContrast(colors){
    let maximumContrast = 0;
    let contrastingColors = null;
    for(let i = 0; i < colors.length; i++){
      for(let j = i+1; j < colors.length; j++){
        let c = Palette.contrast(colors[i],colors[j]);
        if (c > maximumContrast){
          maximumContrast = c;
          contrastingColors = [colors[i],colors[j]];
        }
      }
    }
    return {maximumContrast, contrastingColors};
  }

  static findContrastPairs(colors,minimumContrast){
    let contrastingColors = [];
    for(let i = 0; i < colors.length; i++){
      for(let j = i+1; j < colors.length; j++){
        let c = Palette.contrast(colors[i],colors[j]);
        if (c > minimumContrast){
          contrastingColors.push({foreground: colors[i],background: colors[j]});
        }
      }
    }
...
Palette.inkyColorPalette = (count) => {
  let p = Palette.sevenColorPalette();
  if(typeof count === 'undefined'){
    count = Math.random()*(p.colors.length-2)+2;
  }
  while (p.colors.length > count) {
    p.colors.splice(
      Math.floor(
        Math.random() * p.colors.length
        ),
      1)
  }
  return p;
}
Palette.inkyInterpolatePalette = (count) => {
	let p = Palette.inkyColorPalette();
	if(typeof count === 'undefined'){
	...
	let originalLength = p.colors.length;
	for (var i = 0; i < originalLength; i++) {
		for (var j = i+1; j < originalLength; j++) {
			for (var k = 0; k < count; k++) {
				let lerpIndex = (k+1) / (count+1);
				let newColor = lerpColor(p.colors[i],p.colors[j],lerpIndex);
