class Walker {
  constructor(flowfield, palette, lifespan, group, sizeMultiplier) {
  ...
  draw(graphics) {
    ...
    for (var i = 0; i < this.history.length; i += 3) {
      graphics.ellipse(this.history[i], this.history[i + 1], this.history[i + 2]);
    }
  }
}
